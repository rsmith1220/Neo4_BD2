...
clean_slate = "match (n:Person) detach delete n"
execute_query(clean_slate)
...
def update_node_query(labels, properties, new_properties):
    ...
    property_str = " AND ".join(f"n.{key} = {value}" for key, value in formatted_properties.items())

    formatted_new_properties = {
        key: f"'{value}'" if isinstance(value, str) else str(value)
        for key, value in new_properties.items()
    }
    new_property_str = ", ".join(f"n.{key} = {value}" for key, value in formatted_new_properties.items())
...
# MATCH (n:Person) WHERE n.name = 'John' REMOVE n.salary
def delete_node_properties_query(labels, properties, properties_to_delete):
    ...
    property_str = " AND ".join(f"n.{key} = {value}" for key, value in formatted_properties.items())
    properties_to_delete_str = ", ".join(f"n.{key}" for key in properties_to_delete)
...
def delete_relationship_properties_query(de, a, relationship, properties_to_delete):
    ...
    formatted_properties_to_delete = ", ".join(f"r.{key}" for key in properties_to_delete)
